...
class Controller:
    ...
    def model_states_callback(self, data):
        self.current_x=data.pose[3].position.x
        self.current_y=data.pose[3].position.y
        quaternion = (
            data.pose[3].orientation.x,
            data.pose[3].orientation.y,
            data.pose[3].orientation.z,
            data.pose[3].orientation.w
        )
        euler = tf.transformations.euler_from_quaternion(quaternion)
        self.current_theta= euler[2]
        self.update_costmap([[data.pose[i].position.x,data.pose[i].position.y] for i in [1,2,4]])
    ...
    def controller(self):
        rospy.init_node('controller_node3', anonymous=False)
        tfBuffer = tf2_ros.Buffer()
        listener = tf2_ros.TransformListener(tfBuffer)

        cmd_vel_pub = rospy.Publisher('volta_3/nav/cmd_vel', Twist, queue_size=10)
        rospy.Subscriber('/gazebo/model_states', ModelStates, self.model_states_callback,queue_size=1)
        rospy.Subscriber('/bot3/goal', PoseStamped, self.goal_callback,queue_size=1)
        rospy.sleep(1)
