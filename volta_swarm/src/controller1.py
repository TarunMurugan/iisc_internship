...
class Controller:
    ...
    def model_states_callback(self, data):
        self.current_x=data.pose[1].position.x
        self.current_y=data.pose[1].position.y
        quaternion = (
        ...
        self.update_costmap([[data.pose[i].position.x,data.pose[i].position.y] for i in [2,3,4]])
    ...
    def goal_callback(self, data):
        ...
        self.final_goal[0] = data.pose.position.x       #-1.2*np.sin(self.final_goal[2])
        self.final_goal[1] = data.pose.position.y         #+1.2*np.cos(self.final_goal[2])
    ...
    def controller(self):
        rospy.init_node('controller_node1', anonymous=False)
        tfBuffer = tf2_ros.Buffer()
        listener = tf2_ros.TransformListener(tfBuffer)

        cmd_vel_pub = rospy.Publisher('volta_1/nav/cmd_vel', Twist, queue_size=10)
        rospy.Subscriber('/gazebo/model_states', ModelStates, self.model_states_callback,queue_size=1)
        rospy.Subscriber('/bot1/goal', PoseStamped, self.goal_callback,queue_size=1)
        rospy.sleep(1)
        ...
        while not rospy.is_shutdown():
            # if rospy.get_time()-self.time_prev>0.5:
            #     print("in")
            #     plt.imshow(self.costmap,interpolation='none',extent=[-10,10,10,-10])
            #     plt.show()
