...
class Controller:
    ...
    def model_states_callback(self, data):
        self.current_x=data.pose[4].position.x
        self.current_y=data.pose[4].position.y
        quaternion = (
        ...
        self.update_costmap([[data.pose[i].position.x,data.pose[i].position.y] for i in [1,2,3]])
    ...
    def controller(self):
        rospy.init_node('controller_node4', anonymous=False)
        tfBuffer = tf2_ros.Buffer()
        listener = tf2_ros.TransformListener(tfBuffer)

        cmd_vel_pub = rospy.Publisher('volta_4/nav/cmd_vel', Twist, queue_size=10)
        rospy.Subscriber('/gazebo/model_states', ModelStates, self.model_states_callback,queue_size=1)
        rospy.Subscriber('/bot4/goal', PoseStamped, self.goal_callback,queue_size=1)
        rospy.sleep(1)
